...
},
{
  ...
  "body": " How Newton's Method Works  Since Newton's method is used to approximate a root\/zero\/ -intercept of a differentiable function, we must define the function for which we are trying to solve the equation .  In order to get a visual understanding of how Newton's method works, we begin with a graph of .  !!Picture goes here!!  Newton's method begins with the user making an initial guess for the -intercept being sought. We call this initial guess .  Next, we identify the point on the function as well as the tangent line to the function at that point.  !!Picture goes here!!  Newton's method uses the -intercept of this line as the next approximation to the root being sought, labelling it .  "
},
{
  ...
  "body": " Creating the Formula  Here we will derive the Newton's method recursive formula.  We know that the equation of this tangent line is:     (Make the above an exercise.)  We are interested in finding the -intercept of the tangent line above; we will call this -intercept . Note that its coordinates are . Using these coordinates, together with the tangent line formula above, we can give a formula for in terms of :     "
},
{
